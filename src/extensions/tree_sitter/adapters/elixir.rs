...
const DEF_KWS: &[&str] = &["def", "defp", "defmacro", "defmacrop", "defguard", "defguardp"];
...
fn find_callees(source: &str, lang: &Language, range: &ByteRange) -> Vec<Callee> {
    query_captures(source, lang, "(call target: (identifier) @callee)", "callee", Some(range))
}

fn elixir_kw<'a>(call: Node, source: &'a str) -> Option<&'a str> {
    let target = call.child_by_field_name("target")?;
    if target.kind() == "identifier" { Some(node_text(target, source)) } else { None }
}
...
fn elixir_walk_block(block: Node, source: &str, symbols: &mut Vec<Symbol>, parent: Option<&str>) {
    for i in 0..block.named_child_count() as u32 {
        ...
        let Some(kw) = elixir_kw(c, source) else { continue };

        if MOD_KWS.contains(&kw) {
            if let Some(name) = elixir_mod_name(c, source) {
                let kind = if kw == "defprotocol" { SymbolKind::Interface } else { SymbolKind::Class };
                symbols.push(Symbol {
                    kind, name: name.clone(),
                    range: node_range(c), signature: node_signature(c, source),
                    is_exported: true, parent_class: parent.map(|s| s.to_string()),
                });
            ...
            && let Some(name) = elixir_func_name(c, source) {
                symbols.push(Symbol {
                    ...
                    range: node_range(c), signature: node_signature(c, source),
                    is_exported: !kw.ends_with('p'), parent_class: parent.map(|s| s.to_string()),
                });
            }
    }
}
