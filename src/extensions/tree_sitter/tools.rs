...
//!
//! All synchronous — GrammarManager operations are sync, tools are called from
//! async AgentTool::execute but the actual work is sync.
...
// ── Extract symbols from a file (sync) ──────────────────────────────────
...
fn extract_file(
    ...
    let result = manager.with_lang(&ext, |lang| (entry.extract)(&source, lang));
    let extracted = result.ok()??;
    Some((path.display().to_string(), extracted.symbols))
}
...
impl ListSymbolsTool {
    pub fn new(manager: Arc<GrammarManager>) -> Self { Self { manager } }
}

#[async_trait]
impl AgentTool for ListSymbolsTool {
    fn name(&self) -> &str { "list_symbols" }
    fn label(&self) -> &str { "List Symbols" }
    ...
    async fn execute(&self, params: serde_json::Value, _ctx: ToolContext) -> Result<ToolResult, ToolError> {
        let filter_kind = params.get("kind").and_then(|v| v.as_str()).map(|s| s.to_lowercase());
        if let Some(path_str) = params.get("path").and_then(|v| v.as_str()) {
            let path = Path::new(path_str);
            let syms = extract_file(path, &self.manager).map(|(_, s)| s).unwrap_or_default();
            let syms = filter_symbols(syms, filter_kind.as_deref());
            Ok(ToolResult {
                content: vec![yoagent::types::Content::Text { text: format_symbols(&syms, path_str) }],
            ...
            for file in &files {
                if let Some((path_str, syms)) = extract_file(file, &self.manager) {
                    let syms = filter_symbols(syms, filter_kind.as_deref());
                    if !syms.is_empty() { all_syms.push((path_str, syms)); }
                }
            }
            let total: usize = all_syms.iter().map(|(_, s)| s.len()).sum();
            Ok(ToolResult {
                content: vec![yoagent::types::Content::Text { text: format_all_symbols(&all_syms) }],
...
impl AgentTool for FindDefinitionTool {
    ...
    fn label(&self) -> &str { "Find Definition" }
    fn description(&self) -> &str {
    ...
    async fn execute(&self, params: serde_json::Value, _ctx: ToolContext) -> Result<ToolResult, ToolError> {
        let name = params["name"].as_str().ok_or_else(|| ToolError::InvalidArgs("Missing 'name'".into()))?;
        let hits = find_symbols_globally(name, &self.manager)?;
        if hits.is_empty() {
            return Ok(ToolResult {
                content: vec![yoagent::types::Content::Text { text: format!("No definition found for '{name}'") }],
        ...
        for (path, sym) in &hits {
            lines.push_str(&format!("\n  {path}:{} [{}] {}", sym.range.start_line, sym.kind, sym.signature));
        }
        Ok(ToolResult {
...
impl AgentTool for GetSymbolBodyTool {
    fn name(&self) -> &str { "get_symbol_body" }
    fn label(&self) -> &str { "Get Symbol Body" }
    fn description(&self) -> &str {
    ...
    async fn execute(&self, params: serde_json::Value, _ctx: ToolContext) -> Result<ToolResult, ToolError> {
        let path_str = params["path"].as_str().ok_or_else(|| ToolError::InvalidArgs("Missing 'path'".into()))?;
        let name = params["name"].as_str().ok_or_else(|| ToolError::InvalidArgs("Missing 'name'".into()))?;
        let path = Path::new(path_str);
        ...
        let source = read_file_safe(path).ok_or_else(|| ToolError::Failed(format!("Could not read {path_str}")))?;
        let body = &source[sym.range.start_byte..sym.range.end_byte];
        let line_count = body.lines().count();
        let ext = path.extension().and_then(|e| e.to_str()).unwrap_or("");

        Ok(ToolResult {
            content: vec![yoagent::types::Content::Text { text: format!("Symbol: {name} in {path_str}\n\n{body}") }],
...
impl AgentTool for FindCallersTool {
    ...
    fn label(&self) -> &str { "Find Callers" }
    fn description(&self) -> &str {
    ...
    async fn execute(&self, params: serde_json::Value, _ctx: ToolContext) -> Result<ToolResult, ToolError> {
        let name = params["name"].as_str().ok_or_else(|| ToolError::InvalidArgs("Missing 'name'".into()))?;
        let cwd = std::env::current_dir().unwrap_or_default();
        let search_dir = params.get("path").and_then(|v| v.as_str()).map(Path::new).unwrap_or(&cwd);

        let files = find_project_files(search_dir, 2000);
        let mut callers: Vec<(String, Symbol)> = Vec::new();

        for file in &files {
            let Some((path_str, syms)) = extract_file(file, &self.manager) else { continue };
            for sym in &syms {
                if sym.name == name { continue; }
                if let Some(callees) = callees_for_symbol(file, sym, &self.manager)
        ...
        if callers.is_empty() {
            return Ok(ToolResult {
                content: vec![yoagent::types::Content::Text { text: format!("No callers found for '{name}'") }],
        ...
        for (path, sym) in &callers {
            lines.push_str(&format!("\n  {path}:{} [{}] {}", sym.range.start_line, sym.kind, sym.name));
        }
        Ok(ToolResult {
            content: vec![yoagent::types::Content::Text { text: lines }],
...
impl AgentTool for FindCalleesTool {
    fn name(&self) -> &str { "find_callees" }
    fn label(&self) -> &str { "Find Callees" }
    fn description(&self) -> &str {
    ...
    async fn execute(&self, params: serde_json::Value, _ctx: ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let callees = callees_for_symbol(path, sym, &self.manager).unwrap_or_default();
        if callees.is_empty() {
            return Ok(ToolResult {
                content: vec![yoagent::types::Content::Text { text: format!("No callees found for '{name}'") }],
...
fn find_symbols_globally(name: &str, manager: &GrammarManager) -> Result<Vec<(String, Symbol)>, ToolError> {
    let cwd = std::env::current_dir().unwrap_or_default();
    let files = find_project_files(&cwd, 2000);
    let mut hits = Vec::new();
    for file in &files {
        if let Some((path_str, syms)) = extract_file(file, manager) {
            for sym in syms {
                if sym.name == name {
...
fn filter_symbols(syms: Vec<Symbol>, kind: Option<&str>) -> Vec<Symbol> {
    match kind {
        Some(k) => syms.into_iter().filter(|s| s.kind.to_string() == k).collect(),
        None => syms,
    }
}

fn format_symbol(sym: &Symbol) -> String {
    let class = sym.parent_class.as_ref().map(|c| format!(" [class: {c}]")).unwrap_or_default();
    let export = if sym.is_exported { " (exported)" } else { "" };
    format!("  {}-{} [{}] {}{}{}", sym.range.start_line, sym.range.end_line, sym.kind, sym.name, class, export)
}

fn format_symbols(syms: &[Symbol], file_path: &str) -> String {
    if syms.is_empty() { return format!("No symbols found in {file_path}"); }
    let mut out = format!("## {file_path}");
    for sym in syms { out.push('\n'); out.push_str(&format_symbol(sym)); }
    out.push_str(&format!("\n\n{} symbols", syms.len()));
    out
}
...
fn format_all_symbols(all: &[(String, Vec<Symbol>)]) -> String {
    ...
    for (path, syms) in all {
        ...
        for sym in syms { out.push_str(&format_symbol(sym)); out.push('\n'); }
        total += syms.len();
    }
