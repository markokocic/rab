...
use crate::overlay::{OverlayOptions, OverlayPosition};
use crate::screen::Screen;
use crate::terminal_colors::{TerminalColorScheme, parse_osc11_background_color};
use crate::util::normalize_terminal_output;
...
impl TUI {
    pub fn new() -> Self {
        Self {
            ...
            height: 24,
            dirty: true,
            focused: crate::FocusTarget::None,
    ...
    pub fn set_focus(&mut self, target: crate::FocusTarget) {
        self.clear_focused();
        match target {
            crate::FocusTarget::None => {}
            crate::FocusTarget::Editor => {
                if let Some(ref mut cb) = self.set_editor_focus {
                    cb(true);
                }
            }
            crate::FocusTarget::Overlay(id) => {
    ...
    pub fn show_overlay(&mut self, component: Box<dyn Component>, options: OverlayOptions) -> u64 {
        ...
        if !non_capturing {
            ...
            self.root.set_overlay_focused(id, true);
            self.focused = crate::FocusTarget::Overlay(id);
        }
    ...
    pub fn show_positioned_overlay(
        ...
        use crate::overlay::{OverlayAnchor, OverlayOptions, SizeValue};
    ...
    pub fn hide_overlay(&mut self, id: u64) {
        if self.focused == crate::FocusTarget::Overlay(id) {
            self.clear_focused();
        }
    ...
    pub fn pop_overlay(&mut self) {
        if let crate::FocusTarget::Overlay(_id) = self.focused {
            self.clear_focused();
        }
    ...
    pub fn feed_raw_terminal_data(&mut self, data: &str) -> bool {
        ...
        if crate::terminal_colors::is_osc11_background_color_response(data) {
            if let Some(color) = parse_osc11_background_color(data) {
                let scheme = crate::terminal_colors::color_scheme_from_background(&color);
        ...
        if let Some(scheme) = crate::terminal_colors::parse_terminal_color_scheme_report(data) {
            for listener in &mut self.terminal_color_scheme_listeners {
                listener(scheme);
            }
            return true;
...
mod tests {
    ...
    fn test_overlay_layout_center_default() {
        ...
        let child = crate::components::Text::new("test", 0, 0, None);
        c.show_overlay(
            Box::new(child),
            OverlayOptions::default(),
            crate::FocusTarget::None,
    ...
    fn test_overlay_layout_percent_width() {
        let mut c = Container::new();
        c.set_term_height(24);
        let child = crate::components::Text::new("x", 0, 0, None);
        c.show_overlay(
            Box::new(child),
            OverlayOptions {
    ...
    fn test_overlay_layout_margin() {
        ...
        c.show_overlay(
            Box::new(child),
            OverlayOptions {
                margin: Some(crate::overlay::OverlayMargin {
                    top: 2,
                    right: 2,
                    bottom: 2,
                    left: 2,
