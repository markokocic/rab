...
use crate::focusable::{CURSOR_MARKER, Focusable};
...
use crate::keys::key_event_to_string;
use crate::kill_ring::KillRing;
use crate::util::is_whitespace_char;
use std::collections::HashMap;

use crate::undo_stack::UndoStack;
use crate::util::{visible_width, visual_col_to_byte_offset, wrap_text_with_ansi};
use crate::word_nav::{WordNavigationOptions, find_word_backward_with, find_word_forward_with};
...
impl Editor {
    pub fn new(options: EditorOptions) -> Self {
        Self {
            ...
            border_color: crate::Style::default(),
    ...
    fn set_autocomplete_with_layout(
        ...
        layout: Option<crate::components::select_list::SelectListLayoutOptions>,
    ) {
        if items.is_empty() {
            self.autocomplete_active = false;
            self.autocomplete_list = None;
        ...
        let theme = SelectListTheme {
            ...
            scroll_info: crate::Style::new().fg("\x1b[38;2;128;128;128m".to_string()),
    ...
    fn trigger_autocomplete(&mut self, force: bool) {
        ...
        let layout = if prefix.starts_with('/') {
            Some(crate::components::select_list::SelectListLayoutOptions {
    ...
    fn build_visual_line_spans(&self, width: usize) -> Vec<(usize, usize, usize)> {
        ...
        for (i, line) in self.lines.iter().enumerate() {
            ...
            if line.is_empty() {
                ...
                let chunks = crate::util::wrap_text_with_ansi(line, width);
                let mut byte_pos = 0;
                for chunk in &chunks {
...
impl Component for Editor {
    ...
    fn as_focusable(&mut self) -> Option<&mut dyn crate::Focusable> {
        Some(self)
    }
}
...
mod tests {
    ...
    fn test_content_width_respected() {
        ...
        for line in &lines {
            let vw = crate::util::visible_width(line);
    ...
    fn test_no_duplicate_chunks_from_wrapping() {
        ...
        for text in &texts {
            for width in [1, 2, 3, 5, 8, 12, 20, 40] {
                let wrapped = crate::util::wrap_text_with_ansi(text, width);
    ...
    fn test_visual_col_to_byte_offset_ascii() {
        let text = "hello";
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 0), 0);
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 3), 3);
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 5), 5);
    }
    ...
    fn test_visual_col_to_byte_offset_cjk() {
        ...
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 2), 3);
        // "世界" is width 4, 6 bytes
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 4), 6);
    }
    ...
    fn test_visual_col_to_byte_offset_ansi() {
        ...
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 0), 5); // "h" at byte 5
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 1), 6); // "e" at byte 6
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 2), 7); // first "l" at byte 7
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 3), 8); // second "l" at byte 8
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 4), 9); // "o" at byte 9
        assert_eq!(crate::util::visual_col_to_byte_offset(text, 5), 14); // end at byte 14
    }

    #[test]
    fn test_visual_col_to_byte_offset_empty() {
        assert_eq!(crate::util::visual_col_to_byte_offset("", 0), 0);
        assert_eq!(crate::util::visual_col_to_byte_offset("", 5), 0);
    }

    #[test]
    fn test_visual_col_to_byte_offset_zero_col() {
        // Plain ASCII: first visible char is at byte 0
        assert_eq!(crate::util::visual_col_to_byte_offset("abc", 0), 0);
        // ANSI-prefixed: first visible char is after the ANSI code
        assert_eq!(crate::util::visual_col_to_byte_offset("\x1b[31mabc", 0), 5);
    }
    ...
    fn test_wrap_edge_cases_no_empty_lines() {
        ...
        for (text, width, label) in &cases {
            ...
            for chunk in &wrapped {
                ...
                let vis = crate::util::visible_width(chunk);
                assert!(
                    vis > 0,
    ...
    fn test_wrap_long_word_no_duplicate_chunks() {
        ...
        for width in [5, 6, 7, 8, 10, 12] {
            let wrapped = crate::util::wrap_text_with_ansi(long, width);
